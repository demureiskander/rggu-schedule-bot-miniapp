...
## Cache-busting
Статика на Pages кэшируется (вебвью Telegram — агрессивно). Все локальные импорты
и `index.html` несут `?v=N`. **При каждом деплое бампать версию по всему графу:**
```
perl -0pi -e 's{\?v=(\d+)}{"?v=".($1+1)}ge' src/*.js index.html
```
(плюс при крупных релизах — `?v=N` к Mini App URL в BotFather).
